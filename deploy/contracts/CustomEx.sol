...
contract CustomDEX {
    // custom token to be initialized
    ...
    // Mapping to maintain the tokens and its instances
    ...
    //     // Calculate the amount of output tokens based on the real-time price
    //     uint256 amountOut = (amountIn * uint256(price)) / 1e8; // Assuming 8 decimal places in the price feed

    //     // Approve the router to spend the input token
    //     IERC20(tokenIn).approve(address(uniswapRouter), amountIn);

    //     // Specify Uniswap swap path
