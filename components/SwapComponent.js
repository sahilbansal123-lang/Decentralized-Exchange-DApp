...
const SwapComponent = () => {
  ...
  const ENTER_AMOUNT = "Enter the Amount";
  ...
  const notifySucess = () => toast.success("Transaction Completed");
  ...
  useEffect(() => {
    if (
      ...
      document.activeElement !== "srcToken" &&
      !isReversed.current
    )
  ...
  return (
    <div
      className="border-[1px] rounded-l border-[#7765F3] w-[500px] p-4
    px-7 rounded-xl"
    >
      ...
      <div
        className="relative bg-[#212429] p-4 py-6 rounded-xl mb-2 border-[2px] 
      border-transparent hover:border-zinc-600"
      >
        {srcTokenComp}
        <ArrowSmDownIcon
          className="absolute left-1/2 -transalate-x-1/2 -bottom-6 h-10 p-1 
        bg-[#212429] border-4 border-zinc-900 text-zinc-300 rounded-xl cursor-pointer
        hover:scale-110"
      ...
      <div
        className=" bg-[#212429] p-4 py-6 rounded-xl mt-2 border-[2px] 
      border-transparent hover:border-zinc-600"
      >
        {destTokenComp}
      </div>
  ...
  async function handleIncreaseAllowance() {
    setTxPending(true);
    await increraseAllowance(srcToken, inputValue);
    setTxPending(false);
